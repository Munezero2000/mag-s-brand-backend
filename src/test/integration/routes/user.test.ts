...
describe('api/users', () => {
  ...
  describe('GET /', () => {
    ...
    it('should return a list of users (200) if the authenticated user is authorized', async () => {
      const users = await User.insertMany([{ username: "Munezero", email: "munezero@gmail.com", password: "Mune@123", role: "admin" }, { username: "Mune", email: "mune@gmail.com", password: "Mune@123", role: "reader" }])
  ...
  describe('PUT /:id', () => {
    ...
    it("should return 400 if the user id is invalid or not a valid mongoose id", async () => {
      ...
      await user.save();
      const token = generateAuthToken(user._id);
      const res = await request(server).put('/api/users/:1').set('x-auth-token', token).send({ username: "test username", email: 'test@gmail.com', password: 'password@123', role: "reader" });
      expect(res.status).toBe(400);
    });
  ...
  describe('DELETE /:id', () => {
    it('should return 400 if the user id is missing', async () => {
      const user = new User({ username: "test", email: 'test@test.com', password: 'password@123', role: "reader" });
      await user.save();
      const token = generateAuthToken(user._id);
      const res = await request(server)
        .delete('/api/users/1')
